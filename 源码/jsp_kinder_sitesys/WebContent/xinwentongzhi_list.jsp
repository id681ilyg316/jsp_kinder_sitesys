 ...
 %>
  ...
  <form name="form2" action="delall.jsp">
	...
	%>
        ...
        <td width="60" align="center"><a href="xinwentongzhi_updt.jsp?id=<%=map.get("id")%>&lb=<%=map.get("leibie")%>">修改</a> <a onClick="return confirm('确定要删除这条记录吗?')"  href="xinwentongzhi_list.jsp?scid=<%=map.get("id")%>&lb=<%=map.get("leibie")%>">删除</a></td>
