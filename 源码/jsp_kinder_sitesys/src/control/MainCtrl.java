	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(ac.equals("login"))
		{
			...
			if(!pagerandom.equals(random)&&request.getParameter("a")!=null)
			{
				...
				if (utype.equals("家长"))
				{
					sql1 = "select * from xuesheng where xuehao='"+username+"' and mima='"+password+"' and issh='是'"; 
				
				}
				else
				{
					sql1 = "select * from jiaoshi where zhigongbianhao='"+username+"' and mima='"+password+"' and issh='是'"; 
				}
			...
			if(userlist1.size()==1)
			{
				...
				{
					request.getSession(). setAttribute("username", userlist1.get(0).get("zhigongbianhao"));  
				}
		...
		if(ac.equals("uppass"))
		{
			...
			HashMap m = dao.getmaps("zhigongbianhao",(String)request.getSession().getAttribute("username"), "jiaoshi");
			...
			String sql = "update jiaoshi set mima='"+userpass+"' where zhigongbianhao='"+(String)request.getSession().getAttribute("username")+"'";
			dao.commOper(sql);
			request.setAttribute("suc", "");
			go("mod2.jsp", request, response);
			}
		}
		if(ac.equals("uppassxx"))
		{
			String olduserpass = request.getParameter("ymm");
			String userpass = request.getParameter("xmm1");
			String copyuserpass = request.getParameter("xmm2");   
			HashMap m = dao.getmaps("xuehao",(String)request.getSession().getAttribute("username"), "xuesheng");
			if(!(((String)m.get("mima")).equals(olduserpass)))
			{
				request.setAttribute("error", "");
				go("mod2.jsp", request, response);
			...
			//String id = (String)user.get("id");
			String sql = "update xuesheng set mima='"+userpass+"' where xuehao='"+(String)request.getSession().getAttribute("username")+"'";
			dao.commOper(sql);
			request.setAttribute("suc", "");
			go("mod2.jsp", request, response);
			}
